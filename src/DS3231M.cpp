...
* @details   Class Constructor for DateTime instantiates the class. This is an overloaded class constructor so there
*            are multiple definitions. This implementation ignores time zones and DST changes. It also ignores leap
*            seconds, see http://en.wikipedia.org/wiki/Leap_second. This version uses the compiled in Date and Time
* @param[in] date Character buffer with year value in MMM DD YYYY format
* @param[in] time Character buffer with time value in HH:MM:SS format
******************************************************************************************************************/
...
{
  int8_t _control = readByte(DS3231M_CONTROL&0xE3);
  switch (freq) {
    case  _1hz:
      break;
    case  _1khz:
      ...
      break;
    case  _4khz:
      _control = _control|0x10;
      break;
    case  _8khz:
      _control = _control|0x18;
      break;
  }
  writeByte(DS3231M_CONTROL,_control);
} // of method pinSquareWave(const int8_t freq)
