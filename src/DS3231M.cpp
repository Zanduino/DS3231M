...
{ 
  /*************************************************************************************************************//*!
  * @brief     Class Constructor (Overloaded)
  * @details   Class Constructor for DateTime instantiates the class. This is an overloaded class constructor so
  *            there are multiple definitions. This implementation ignores time zones and DST changes. It also 
  *            ignores leapseconds, see http://en.wikipedia.org/wiki/Leap_second
  * @param[in] year Year
...
DateTime::DateTime (const char* date, const char* time)
{
  ...
  * @details   Class Constructor for DateTime instantiates the class. This is an overloaded class constructor so there
  *            are multiple definitions. This implementation ignores time zones and DST changes. It also ignores leap
  *            seconds, see http://en.wikipedia.org/wiki/Leap_second. This version uses a string representation of the
  *            date and time to instantiate. The date and time arrive in string format as follows: date = "Dec 26 2009",
  *            time = "12:34:56"
...
DateTime::DateTime (const __FlashStringHelper* date, const __FlashStringHelper* time)
{
  ...
  * @details   Class Constructor for DateTime instantiates the class. This is an overloaded class constructor so 
  *            there are multiple definitions. This implementation ignores time zones and DST changes. It also 
  *            ignores leap seconds, see http://en.wikipedia.org/wiki/Leap_second. This version uses the compiled 
  *            in Date and Time
  * @param[in] date Character buffer with year value in MMM DD YYYY format
  * @param[in] time Character buffer with time value in HH:MM:SS format
  ****************************************************************************************************************/
...
bool DS3231M_Class::begin(const uint32_t i2cSpeed)
{
  ...
  ****************************************************************************************************************/
  ...
  {
    writeByte(DS3231M_RTCHOUR,readByte(DS3231M_RTCHOUR)&B10111111); // Force use of 24 hour clock by turning off bit
...
uint8_t DS3231M_Class::bcd2int(const uint8_t bcd)
{
  ...
  * @details   Takes a byte of BCD (Binard coded Decimal) and converts to the binary integer representation
...
void DS3231M_Class::adjust()
{
  ...
  * @details   This is an overloaded function, if called with no parameters then the RTC is set to the date/time when
  *            the program was compiled and uploaded. Otherwise the values are set, but the oscillator is stopped
  *            during the process and needs to be restarted upon completion
  ******************************************************************************************************************/
...
void DS3231M_Class::adjust(const DateTime& dt)
{
  ...
  *****************************************************************************************************************/
  ...
  writeByte(DS3231M_RTCHOUR,int2bcd(dt.hour()%24));               // Also re-sets the 24Hour clock on
...
void DS3231M_Class::setAlarm(const uint8_t alarmType, const DateTime dt, const bool state ) 
{
  ...
  * @details   As the two alarms aren't identical in what will trigger them, this call chooses which alarm to set 
  *            depending upon the "alarmType" parameter value.
  ...
  ****************************************************************************************************************/
  ...
  if (alarmType<everyMinute) // These types go to alarm 1
  {
    ...
    if(alarmType<secondsMinutesHoursDateMatch)                    // Set the high-bit of ALM1DATE if the alarm bit needs setting
      writeByte(DS3231M_ALM1DATE,readByte(DS3231M_ALM1DATE)|0x80);
    if(alarmType<secondsMinutesHoursMatch)                        // Set the high-bit of ALM1HOUR if the alarm bit needs setting
      writeByte(DS3231M_ALM1HOUR,readByte(DS3231M_ALM1HOUR)|0x80);
    if(alarmType<secondsMinutesMatch)                             // Set the high-bit of ALM1MIN if the alarm bit needs setting
...
uint8_t DS3231M_Class::weekdayRead()
{
  ...
  * @details This number is user-settable and is incremented when the day shifts. It is set as part of the adjust()
  *          method where Monday is weekday 1
